...
void SmartMinimizor::Minimize(
    ...
    population.clear();
    population.insert(Solution(sched.GetStrategy(), nonlinear_histories, total_tasks));

    // TODO: when the mutations count is 1, we could add a counter which will increase,
    ...
    // replay the round with found nonlinearized interleaving, this put the round in correct final state and builds a `BothHistories` object
    auto replayed_result = sched.ReplayRound(StrategyScheduler::GetTasksOrdering(best_solution.nonlinear_histories.first, {}));
...
void SmartMinimizor::DropRandomTask(std::unordered_map<int, std::unordered_set<int>>& threads) const {
    ...
    for (int offspring = 1; offspring <= offsprings_per_generation; ++offspring) {
      ...
      while (left_attempts--) {
        ...
        for (int m = 1; m <= mutations_count; ++m) {
          ...
          // no mutations will be applyed at all. This is done to sometimes generate offspring
          // that mixes the parent threads without changing the tasks count.
...
const Solution* p2
    ...
    //     (the smaller the tasks count, the bigger probability of selecting this thread)
    //  2. if threads count differs, then we could compare the tasks count with
    //     another thread index (e.g. if p1: [T1, T2] and p2: [T1, T2, T3, T4], then compare
    //     p1:T2 not only with p2:T2, but with p2:T3 and p2:T4 as well)
...
float SmartMinimizor::Solution::GetFitness() const {
    return fitness;
}

int SmartMinimizor::Solution::GetValidTasks() const {
    return valid_tasks;
}