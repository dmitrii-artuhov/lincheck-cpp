...
struct Scheduler {
  using FullHistory = std::vector<std::variant<Task, DualTask>>;
  using SeqHistory = std::vector<HistoryEvent>;
...
//                size_t max_switches, std::vector<TasksBuilder> constructors,
...
//   // TODO: for non obstruction-free we need to take into account
//   dependencies. void TerminateTasks() {
...
//         // Task exists.
//         frame.is_new = false;
//         auto [is_over, res] = ResumeTask(frame, step, switches, thread,
//         false); if (is_over || res.has_value()) {
//           return {is_over, res};
//         }
