...
template <class LinearSpecificationObject, class SpecificationObjectHash = std::hash<LinearSpecificationObject>>
struct PairHash {
  std::uint64_t operator()(const std::pair<std::vector<bool>, LinearSpecificationObject>& pair) const
  {
...
template <class LinearSpecificationObject, class SpecificationObjectEqual = std::equal_to<LinearSpecificationObject>>
struct PairEqual {
  constexpr bool operator()(const std::pair<std::vector<bool>, LinearSpecificationObject>& lhs, const std::pair<std::vector<bool>, LinearSpecificationObject>& rhs) const
  {
    return lhs.first == rhs.first && SpecificationObjectEqual{}(lhs.second, rhs.second);
  }
};

template<class LinearSpecificationObject, class SpecificationObjectHash, class SpecificationObjectEqual>
LinearizabilityChecker<LinearSpecificationObject, SpecificationObjectHash, SpecificationObjectEqual>::
  LinearizabilityChecker(std::map<MethodName, std::function<int(LinearSpecificationObject)>> specification_methods,
                         std::function<LinearSpecificationObject(LinearSpecificationObject)> copy, LinearSpecificationObject first_state) :
                                               specification_methods(specification_methods), copy(copy), first_state(first_state) {}

// Implements the wgl linearizability checker, http://www.cs.ox.ac.uk/people/gavin.lowe/LinearizabiltyTesting/
// https://arxiv.org/pdf/1504.00204.pdf
template<class LinearSpecificationObject, class SpecificationObjectHash, class SpecificationObjectEqual>
bool LinearizabilityChecker<LinearSpecificationObject, SpecificationObjectHash, SpecificationObjectEqual>::
  Check(const std::vector<std::variant<StackfulTaskInvoke, StackfulTaskResponse>> &history) {
  // head entry
  size_t current_section_start = 0;
  LinearSpecificationObject data_structure_state = copy(first_state);
  // indexes of invokes
  std::vector<size_t> open_sections_stack;
  // TODO: Can replace it with stack of hashes and map: hash -> LinearSpecificationObject
  // contains previous states stack
  ...
  while (open_sections_stack.size() != history.size() / 2) {
    // This event is already in the stack, don't need lift function with this predicate
    ...
    if (history[current_section_start].index() == 0) {
      // invoke
      const StackfulTaskInvoke& inv = std::get<0>(history[current_section_start]);
      assert(specification_methods.find(inv.task.GetName()) != specification_methods.end());
      auto method = specification_methods.find(inv.task.GetName())->second;
      // apply method
      bool was_checked = false;
      LinearSpecificationObject data_structure_state_copy = copy(data_structure_state);
      ...
      if (res == inv.task.GetRetVal()) {
        ...
        was_checked = states_cache.find({linearized, data_structure_state_copy}) == states_cache.end();
        if (!was_checked) {
      ...
      // haven't seen this state previously, so continue procedure with this new state
      if (!was_checked) {
        // open section
        ...
        // already seen this state (linearization part and state) or not linearizable, anyway should go to the next one
        // can't append this entry now, have to go to the next one
        current_section_start++;
      }
    } else {
      // A response event
      ...
      data_structure_state = states_stack.top();
